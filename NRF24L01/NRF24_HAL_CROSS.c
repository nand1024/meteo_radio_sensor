 ...
 */
/*
 * "злапано" з бібліотек репозиторію arduino та модифіковано для використання без прив'язки до платформи arduino
 */
#include <string.h>
#include <stdlib.h>
...
static SPI_controls_CB SpiCB;
void hal_nrf_init(SPI_controls_CB *val)
{
    SpiCB.csEnable = val->csEnable;
    SpiCB.csDisable = val->csDisable;
    SpiCB.transferByte = val->transferByte;
    SpiCB.transferByteArray = val->transferByteArray;
    SpiCB.csDisable();
}
...
{
    return hal_nrf_write_reg(STATUS_NRF,
...
uint8_t hal_nrf_clear_irq_flags_get_status(void)
{
    ...
    uint8_t retval = hal_nrf_write_reg(STATUS_NRF,
        (uint8_t)(BIT(MAX_RT)|BIT(TX_DS)|BIT(RX_DR))) &
        (uint8_t)(BIT(MAX_RT)|BIT(TX_DS)|BIT(RX_DR));

    retval |= hal_nrf_read_reg(STATUS_NRF) &
        (uint8_t)~(BIT(MAX_RT)|BIT(TX_DS)|BIT(RX_DR));
...
{
    hal_nrf_write_reg(STATUS_NRF, (uint8_t)BIT(int_source));
}
...
{
    SpiCB.csEnable();
    SpiCB.transferByte(REUSE_TX_PL);
    SpiCB.csDisable();
}
    ...
    hal_nrf_read_multibyte_reg(uint8_t reg, uint8_t *pbuf, uint8_t len)
{
    ...
    if (length > 0) {
        ...
        SpiCB.csEnable();
        SpiCB.transferByteArray(buf, length+1);
        SpiCB.csDisable();
